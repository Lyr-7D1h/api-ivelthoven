...
const github = {
  ...
  auth: `?client_id=${process.GITHUB_CLIENT}&client_secret=${
    process.GITHUB_CLIENT_SECRET
  }`
};
          ...
          for (let i in data) {
            ...
            getJson(repo.deployments_url + github.auth, data => {
              const stmt = db.prepare(
                "REPLACE INTO github(id, name, description, created_at, url, deployments) VALUES (?, ?, ?, ?, ?, ?)",
                err => errHandler(err)
              );
              let is_deploy = false;
